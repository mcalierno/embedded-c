...
static const char wf_ordering_table[ ] =
{
...
*/
static const char ASCI_to_wf_codification_table [ ] =
{
...
void LCDAllSegments(int mode)
{
	...
	while (lbCounter < lbTotalBytes)
	{
		...
		{
			lbpLCDWF[(uint8_t)wf_ordering_table[lbCounter++]] = 0;
		}
	}
}
...
{
	if (char_position >= _CHARNUM)
		char_position = 0;

	if (lbValue == '.')
	{
		// Use built-in dot
		LCDDP(char_position-1, true);
		return;
	}
	...
	{
		position = (char_position) *_LCDTYPE + lbCounter;
		temp=0;

		if (lbCounter==1)
		{
			temp = lbpLCDWF[(uint8_t)wf_ordering_table[position]] & 0x01;//bit 0 has the special symbol information
		}

		char_val = ASCI_to_wf_codification_table[arrayOffset + lbCounter];
		lbpLCDWF[(uint8_t)wf_ordering_table[position]] = char_val | temp;
...
{
	char buffer[_CHARNUM + 1];

	va_list args;
  va_start(args, format);
  vsnprintf(buffer, _CHARNUM + 1, format, args);
